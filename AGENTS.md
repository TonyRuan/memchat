...
- 做测试审查、测试重构或大范围验证时，尽量使用子代理并行拆分，提升速度并减少主上下文污染。
- 测试重构必须以 PRD 第 17、18、20 节和项目真实行为为依据，不默认沿用旧测试的覆盖结论。
- 旧测试如果覆盖不全面、断言不严谨、只验证实现细节或与 PRD 脱节，应重写、拆分或降级为辅助测试。
- 测试分层优先采用：纯 Kotlin 单元测试覆盖领域/仓储/解析逻辑，仪器测试覆盖 Room、加密存储和 Compose 交互，人工或 ADB E2E 覆盖真实模型链路。
- 子代理可负责独立测试域的审查和建议，最终整合、冲突处理和完整验证仍由主线程完成。
